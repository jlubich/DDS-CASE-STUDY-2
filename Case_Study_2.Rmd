...
Additional variables should also be considered, such as reason for attrition. Some of the reasons may include and not be limited to: Retirement, Career Change, Salary. DDSAnalytcis should also review the way employee surveys are built and administered in order to limit anay biases that may exist.
The creation of a predictive model is a logical next step, but DDSAnalytics can also start building an infrastructure for machine learning and prescriptive analytics, which will be natural progressions as it expands upon its use of Data Science.
